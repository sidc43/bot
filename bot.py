...
class MCBot:
    ...
    def start_events(self):
        ...
        def on_message(this, msg, position, json_msg, sender, verified=None):
            if sender == this.username:
                return

            players = this.players
            player = players[sender] if sender in players else None

            if player:
                print(f"Sender: {sender}")
                print(f"UUID: {player['uuid']}")
                print(f"Entity: {player['entity']}")
                print(f"Ping: {player.get('ping', 'unknown')}")
            else:
                print(f"Unknown sender: {sender}")

            this.chat(f"Hi {sender}, I see your message!")
