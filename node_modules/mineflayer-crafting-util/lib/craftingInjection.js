...
async function injectBot(bot, botoptions) {
    // @ts-expect-error
    ...
    function craftWithInventory(wantedItem) {
...
async function buildStatic(registry) {
    // @ts-expect-error
    const Recipe = (await Promise.resolve().then(() => __importStar(require("prismarine-recipe")))).default(registry).Recipe;
